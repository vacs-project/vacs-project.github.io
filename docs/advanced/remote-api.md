...
| `app_open_url` [^desktop-only]                      | `url`: string                             | -                                           | Open a URL in the desktop host's default browser.                                |
...
| `playback_say_again`     | `deviceType`: [`PlaybackDeviceType`](#playbackdevicetype)                                                                                   | [`ClipMeta`](#clipmeta) &#124; `null` | Replay the newest recorded clip from the start, replacing any playing clip. Returns the clip started, or `null` when nothing has been recorded. |
